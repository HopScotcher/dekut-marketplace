...
export function DotNetAdapter(): Adapter {
  return {
    ...
    async createUser(user: Omit<AdapterUser, "id">): Promise<AdapterUser> {
      const response = await adapterClient.post<AdapterUser>(
        "/api/auth/users",
        user
      );
    ...
    async getUser(id: string): Promise<AdapterUser | null> {
      try {
        const response = await adapterClient.get<AdapterUser>(
          `/api/auth/users/${id}`
        );
        return response.data;
      } catch (error: any) {
    ...
    async updateUser(
      ...
      const response = await adapterClient.put<AdapterUser>(
    ...
    async linkAccount(
      ...
      const response = await adapterClient.post<AdapterAccount>(
        "/api/auth/accounts",
        account
      );
      return response.data;
    ...
    async createSession(session: {
      ...
      const response = await adapterClient.post<AdapterSession>(
        "/api/auth/sessions",
        {
          ...session,
